...
class SapCourseExporter(BaseCourseExporter):  # pylint: disable=abstract-method
    ...
    def get_serialized_data_blocks(self):
        ...
        page_length = 1000
        location = 0
        while (location < len(self.courses)) or not location:
            this_batch = self.courses[location:location+page_length]
            yield (
                json.dumps({'ocnCourses': this_batch}, sort_keys=True).encode('utf-8'),
                len(this_batch)
            )
            location += page_length
    ...
    data_transform = {
        ...
        'title': lambda x: [
            {
                'locale': parse_language_code(x['content_language']),
                'value': x['title']
            },
        ],
...
def get_course_track_selection_url(enterprise_customer, course_id):
    """
    Given a course ID, and EnterpriseCustomer, craft a URL that links to the track selection page for that course.
